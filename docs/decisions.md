...
- **Geodata:** generated by a tool from the client's own terrain, meshes and BSP into Canastra's
  ...
  - *2026-09-20:* the server reads High Five geodata as it stands, in the L2J region format, one file per map
    tile (`17_25.l2j`). It follows that format's rules: a cell covers sixteen units and keeps the height of
    each floor over it with the sides it opens onto; a walk steps cell by cell along a straight line, each step
    has to be allowed by the side it leaves through, a diagonal step also needs both straight steps around it,
    and no step may climb more than 48. A walk that cannot be finished ends at the last cell reached. Entering
    the world puts a character on the floor the geodata gives, preferring the floor below where several are
    stacked. Generating our own geodata from the client stays the goal; this reads what servers already have.
